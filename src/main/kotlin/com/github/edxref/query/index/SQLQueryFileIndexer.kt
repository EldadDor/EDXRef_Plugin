  ...
  override fun getInputFilter(): FileBasedIndex.InputFilter {
    ...
    return FileBasedIndex.InputFilter { file: VirtualFile ->
      ...
      if (file.fileType != com.intellij.ide.highlighter.XmlFileType.INSTANCE) {
        // log.trace("InputFilter skipping non-XML: ${file.path}") // Use trace if needed, debug
      ...
      try {
        ...
        if (psiFile is XmlFile) {
          ...
          if (root?.name == "Queries") {
            ...
            log.debug(
              "Root tag is not '<Queries>' (found '${root?.name}') in file '${file.name}'. Skipping tags."
            )
          }
        } else {
          log.debug(
            "File '${file.name}' is not an XmlFile (Type: ${psiFile.javaClass.name}). Skipping."
          )
        }
      } catch (e: Exception) {
