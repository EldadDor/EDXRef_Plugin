...
class QueryIndexService(private val project: Project) {
	...
	private fun performSingleBatchXmlLookup(queryIds: Set<String>): Map<String, PsiElement> {
		...
		try {
			...
			queryIds.forEach { queryId ->
				fileIndex.getValues(SQLQueryFileIndexer.KEY, queryId, GlobalSearchScope.projectScope(project)).forEach { filePath -> candidateFiles.add(filePath) }
			}
	...
	private fun processXmlFileForBatch(
		...
		try {
			val virtualFile = com.intellij.openapi.vfs.LocalFileSystem.getInstance().findFileByPath(filePath) ?: return
	...
	private fun processJavaFileForSQLRefAnnotations(
		...
		try {
			val virtualFile = com.intellij.openapi.vfs.LocalFileSystem.getInstance().findFileByPath(filePath) ?: return emptyMap()

			val psiFile = PsiManager.getInstance(project).findFile(virtualFile) as? PsiJavaFile ?: return emptyMap()
	...
	fun getCacheStats(): Map<String, Any> {
		return mapOf(
			...
			"hitRate" to if (cacheHits + cacheMisses > 0) (cacheHits.toDouble() / (cacheHits + cacheMisses) * 100).toInt()
			else 0,
		)
	}
	...
	fun forceReindex() {
		try {
			...
			fileBasedIndex.requestRebuild(SQLQueryFileIndexer.KEY)
			fileBasedIndex.requestRebuild(SQLRefAnnotationIndex.KEY)
