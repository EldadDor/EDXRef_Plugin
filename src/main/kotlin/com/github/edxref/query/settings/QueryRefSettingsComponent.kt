...
/**
 * Represents the UI panel for QueryRef settings. Contains the Swing components users interact with.
 */
class QueryRefSettingsComponent {
  ...
  init {
    ...
    panel =
      FormBuilder.createFormBuilder()
        ...
        .addTooltip(
          "Glob pattern for query XML file locations (e.g., **/resources/queries/*-queries.xml)"
        )
        .addLabeledComponent(JBLabel("SQLRef Annotation FQN:"), sqlRefAnnotationFqnField, 1, false)
        .addTooltip(
          "Fully qualified name of the annotation marking query references (e.g., com.example.SqlRef)"
        )
        .addLabeledComponent(
          JBLabel("SQLRef Annotation Attribute:"),
          sqlRefAnnotationAttributeNameField,
          1,
          false,
        )
        ...
        .addTooltip(
          "Fully qualified name of the QueryUtils class (e.g., com.github.edxref.QueryUtils)"
        )
        .addLabeledComponent(
          JBLabel("QueryUtils Method Name:"),
          queryUtilsMethodNameField,
          1,
          false,
        )
  ...
  var queryUtilsFqn: String
    ...
    set(value) {
      queryUtilsFqnField.text = value
    }

  var queryUtilsMethodName: String
    get() = queryUtilsMethodNameField.text
    set(value) {
      queryUtilsMethodNameField.text = value
    }
}
