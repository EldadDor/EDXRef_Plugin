...
class ScanInternalApiAction : AnAction() {
  ...
  override fun actionPerformed(e: AnActionEvent) {
    ...
    ProgressManager.getInstance()
      .run(
        object : Task.Backgroundable(project, "Scanning for @InternalApi Usages", true) {
          override fun run(indicator: ProgressIndicator) {
            ...
            val annotationClass =
              ReadAction.compute<PsiClass?, Throwable> {
                JavaPsiFacade.getInstance(project)
                  .findClass(internalApiFqn, GlobalSearchScope.allScope(project))
              }
            ...
            if (annotationClass == null) {
              ...
              showErrorNotification(
                project,
                "@InternalApi annotation class not found: $internalApiFqn",
              )
            ...
            val findUsagesHandler =
              (FindManager.getInstance(project) as FindManagerImpl)
                .findUsagesManager
                .getFindUsagesHandler(annotationClass, false)
            ...
            ApplicationManager.getApplication().invokeLater {
              ...
              if (usages.isEmpty()) {
                showInfoNotification(
                  project,
                  "No usages of @InternalApi ($internalApiFqn) found in project scope.",
                )
                return@invokeLater
              }
              ...
              UsageViewManager.getInstance(project)
                .showUsages(
                  UsageTarget
                    .EMPTY_ARRAY, // Targets are often the element searched for, but UsageInfo has
                  // elements
                  usages,
                  presentation,
                )
