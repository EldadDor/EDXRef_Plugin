...
private fun getWsConsumerAnnotationFqn(project: Project): String {
  ...
  return getSettings(project).wsConsumerAnnotationFqn.ifBlank {
...
private fun getWebserviceConsumerFqn(project: Project): String {
  // Simplified - replace with actual implementation using defaults if needed
  return getSettings(project).webserviceConsumerFqn.ifBlank {
...
private fun getPearlWebserviceConsumerFqn(project: Project): String {
  // Simplified - replace with actual implementation using defaults if needed
  return getSettings(project).pearlWebserviceConsumerFqn.ifBlank {
...
private fun getPropertyFqn(project: Project): String {
  // Simplified - replace with actual implementation using defaults if needed
  return getSettings(project).propertyAnnotationFqn.ifBlank {
    "com.github.edxref.annotations.Property"
  } // Added fallback
}
  ...
  override fun applyFix(project: Project, descriptor: ProblemDescriptor) {
    try {
      ...
      if (isJava && annotationElement is PsiAnnotation) {
        // Java implementation (unchanged, but wrapped in try)
        val factory = JavaPsiFacade.getElementFactory(project)
        val attr =
          factory.createAnnotationFromText(
            "@WSConsumer(url = \"http://localhost:8080/service\", method = com.example.WSMethods.GET)",
            null,
          )
        ...
        val hasMethodAttribute =
          ktAnnotationEntry.valueArguments.any {
          ...
          if (!hasMethodAttribute) {
            ...
            val methodArgument =
              psiFactory.createArgument(
                "method = com.example.WSMethods.GET"
              ) // ADJUST FQN if needed
            argList.addArgument(methodArgument)
          }
        } else {
          val methodText =
            if (hasMethodAttribute) ""
            else ", method = com.example.WSMethods.GET" // ADJUST FQN if needed
      ...
      // Log or handle PSI modification failure in K2
      logger<AddDefaultUrlQuickFix>().error("Failed to apply quick fix in K2 mode: ${e.message}")
    }
  }
}

// --- Helper function to find attribute value for highlighting (CORRECTED AGAIN) ---
private fun findHighlightElementForAttribute(
  ...
  return when (annotationElement) {
    ...
    is KtAnnotationEntry ->
      annotationElement.valueArguments
        .find {
  ...
  fun checkWSConsumerAnnotation(
    ...
    if (urlValue.isEmpty() && pathValue.isEmpty()) {
      ...
      holder.registerProblem(
        annotationElement,
        MyBundle.message(
          "plugin.rules.missing.url.and.path",
          "Either 'url' or 'path' must be specified in @${wsConsumerAnnotationFqn.substringAfterLast('.')} annotation.",
        ),
    ...
    if (hasMsConsumer && urlValue.isNotEmpty()) {
      ...
      holder.registerProblem(
        ...
        MyBundle.message(
          "plugin.rules.url.with.msconsumer",
          "For @${wsConsumerAnnotationFqn.substringAfterLast('.')} with msConsumer, 'url' must not be specified; use 'path' only.",
        ),
        ProblemHighlightType.ERROR,
      )
    }
    ...
    if (
      pathValue.isNotEmpty() && (pathValue.contains("http://") || pathValue.contains("https://"))
    ) {
      ...
      holder.registerProblem(
        ...
        MyBundle.message(
          "plugin.rules.path.with.protocol",
    ...
    if (urlValue.isNotEmpty()) {
      ...
      if (protocolIndex >= 0 && urlValue.substring(protocolIndex + 3).contains("//")) {
        ...
        holder.registerProblem(
          findHighlightElementForAttribute(annotationElement, "url") ?: annotationElement,
          MyBundle.message(
            "plugin.rules.url.double.slashes",
    ...
    if (pathValue.isNotEmpty() && urlValue.isEmpty() && !hasMsConsumer) {
      ...
      holder.registerProblem(
        findHighlightElementForAttribute(annotationElement, "path") ?: annotationElement,
        MyBundle.message(
          "plugin.rules.path.without.msconsumer",
    ...
    if (urlValue.isNotEmpty()) {
      ...
      val invalidHosts =
        invalidHostsStr.split(',').mapNotNull { it.trim().takeIf { it.isNotEmpty() } }
      ...
      for (host in invalidHosts) {
        if (urlValue.contains(host)) {
          ...
          holder.registerProblem(
            findHighlightElementForAttribute(annotationElement, "url") ?: annotationElement,
            MyBundle.message(
              "plugin.rules.invalid.server",
              "Invalid URL: ''{0}'' is in the list of restricted servers.",
              host,
            ),
            ProblemHighlightType.ERROR,
          )
    ...
    if (isPearlWebserviceConsumer && hasMsConsumer) {
      val containsInvalidType =
        msConsumerValue.contains("CRM") ||
      ...
      if (containsInvalidType) {
        ...
        if (containsLocal && sslCertificateValidation) {
          ...
          holder.registerProblem(
            findHighlightElementForAttribute(annotationElement, "sslCertificateValidation")
              ?: annotationElement,
            MyBundle.message(
              "plugin.rules.pearl.ssl.validation",
    ...
    // Rule 7: Disallow PEARL LbMsType for regular WebserviceConsumer
      ...
      holder.registerProblem(
        findHighlightElementForAttribute(annotationElement, "msConsumer") ?: annotationElement,
        MyBundle.message(
          "plugin.rules.non.pearl.consumer",
...
class WSConsumerJavaInspection : AbstractBaseJavaLocalInspectionTool(), WSConsumerInspectionLogic {
  ...
  override fun getDisplayName(): String =
    "WSConsumer Annotation Validation (Java)" // More specific display name
  ...
  override fun checkClass(
    ...
    for (annotation in annotations) {
      if (annotation.qualifiedName == wsConsumerAnnotationFqn) { // Use FQN
        logIfEnabled(
          project,
          logger<WSConsumerJavaInspection>(),
          "Found @${wsConsumerAnnotationFqn.substringAfterLast('.')} annotation, checkClass",
        )
        ...
        if (urlValue == null && pathValue == null) {
          logIfEnabled(
            project,
            logger<WSConsumerJavaInspection>(),
            "plugin.rules.missing.url.and.path (checkClass)",
          )
          return arrayOf(
    ...
    return object : JavaElementVisitor() {
      override fun visitAnnotation(annotation: PsiAnnotation) {
        ...
        logIfEnabled(
          project,
          logger<WSConsumerJavaInspection>(),
          "Found @${wsConsumerAnnotationFqn.substringAfterLast('.')} annotation, visit",
        )
        ...
        val urlValue =
          if (urlAttr is PsiLiteralExpression && urlAttr.value is String) urlAttr.value as String
          else ""

        val pathAttr = annotation.findAttributeValue("path")
        val pathValue =
          if (pathAttr is PsiLiteralExpression && pathAttr.value is String) pathAttr.value as String
          else ""

        val sslValue = annotation.findAttributeValue("sslCertificateValidation")
        val sslCertificateValidation =
          if (sslValue is PsiLiteralExpression && sslValue.value is Boolean)
            sslValue.value as Boolean
          else true
...
// --- Kotlin Implementation ---
class WSConsumerKotlinInspection :
  AbstractBaseUastLocalInspectionTool(), WSConsumerInspectionLogic {
  // No need to override log here, interface provides default

  override fun getDisplayName(): String =
    "WSConsumer Annotation Validation (Kotlin)" // More specific display name

  override fun getStaticDescription(): String =
    "Validates proper usage of @WSConsumer annotation in Kotlin code, checking URL/path specifications, SSL settings, and msConsumer configuration"

  override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean): PsiElementVisitor =
    object : KtVisitorVoid() {
      override fun visitAnnotationEntry(annotationEntry: KtAnnotationEntry) {
        super.visitAnnotationEntry(annotationEntry)
        // Removed wrapping analyze {} - not needed for PSI traversals
        ...
        logIfEnabled(
          project,
          logger<WSConsumerKotlinInspection>(),
          "Found @$shortName annotation, visit",
        )

        val ktClass: KtClass = annotationEntry.getStrictParentOfType() ?: return

        // Updated: Use Analysis API for super type check instead of superTypeListEntries
        var isPearlWebserviceConsumer = false
        analyze(ktClass) { // Minimal analyze {} only for symbol resolution
          val classSymbol = ktClass.getClassOrObjectSymbol()
          isPearlWebserviceConsumer =
            classSymbol?.superTypes?.any { superType: KtType ->
              superType.expandedClassSymbol?.classIdIfNonLocal?.asSingleFqName()?.asString() ==
                getPearlWebserviceConsumerFqn(project)
            } ?: false
        }

        // Helper to extract literal text (unchanged)
        fun getArgumentText(name: String): String {
          val arg =
            annotationEntry.valueArguments.find { it.getArgumentName()?.asName?.asString() == name }
              ?: return ""
          val expr = arg.getArgumentExpression()
          val text = expr?.text ?: ""
          return if (
            expr is KtStringTemplateExpression && text.startsWith("\"") && text.endsWith("\"")
          ) {
        ...
        val sslCertificateValidation =
          if (sslText.isEmpty()) true else sslText.equals("true", ignoreCase = true)

        val msConsumerArg =
          annotationEntry.valueArguments.find {
            it.getArgumentName()?.asName?.asString() == "msConsumer"
          }
        // Simplified hasMsConsumer (as above)
        val msConsumerExpr = msConsumerArg?.getArgumentExpression()
        val hasMsConsumer =
          msConsumerExpr != null &&
            msConsumerExpr.text.isNotBlank() &&
            !msConsumerExpr.text.equals("{}", ignoreCase = true) &&
            !msConsumerExpr.text.equals("[]", ignoreCase = true) &&
            !msConsumerExpr.text.equals("arrayOf()", ignoreCase = true)
        val msConsumerValue = msConsumerExpr?.text ?: ""

        // Call common logic (unchanged)
        checkWSConsumerAnnotation(
          project,
          annotationEntry,
          holder,
