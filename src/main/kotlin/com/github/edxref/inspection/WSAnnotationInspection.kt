	...
	override fun applyFix(project: Project, descriptor: ProblemDescriptor) {
		try {
			...
			if (isJava && annotationElement is PsiAnnotation) {
				...
				val attr = factory.createAnnotationFromText("@WSConsumer(url = \"http://localhost:8080/service\", method = com.example.WSMethods.GET)", null)
		...
		object : KtVisitorVoid() {
			override fun visitAnnotationEntry(annotationEntry: KtAnnotationEntry) {
				...
				analyze(ktClass) {  // Minimal analyze {} only for symbol resolution
					...
					isPearlWebserviceConsumer = classSymbol?.superTypes?.any { superType: KtType ->
						superType.expandedClassSymbol?.classIdIfNonLocal?.asSingleFqName()?.asString() == getPearlWebserviceConsumerFqn(project)
					} ?: false
				}
				...
				val hasMsConsumer = msConsumerExpr != null &&
