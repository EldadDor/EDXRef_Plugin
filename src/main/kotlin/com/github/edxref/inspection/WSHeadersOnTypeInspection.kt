...
interface WSHeadersOnTypeInspectionLogic {
	...
	fun validateTypeHeaders(
		...
		InspectionUtil.logIfEnabled(project, log, "Running WSHeadersOnType validation on ${psiClass.name}")
	...
	fun validateKotlinTypeHeaders(
		...
		InspectionUtil.logIfEnabled(project, log, "Running WSHeadersOnType validation on Kotlin ${classOrObject.name}")
	...
	private fun validateSingleTypeHeaderDefault(
		...
		val headerName = InspectionUtil.getAnnotationStringAttribute(headerAnnotation, "name") ?: "[Unknown Name]"
		...
		InspectionUtil.logIfEnabled(headerAnnotation.project, logger, "Validating type @WSHeader '$headerName' with defaultValueAttr='$defaultValueAttr'")

		if (!isValid) {
			InspectionUtil.logIfEnabled(headerAnnotation.project, logger, "ERROR: Type-level header '$headerName' has missing, empty, or invalid defaultValue.")
	...
	private fun validateSingleKotlinTypeHeaderDefault(
		...
		val headerName = InspectionUtil.getKotlinAnnotationStringAttribute(headerAnnotation, "name") ?: "[Unknown Name]"
		...
		InspectionUtil.logIfEnabled(headerAnnotation.project, logger, "Validating Kotlin type @WSHeader '$headerName' with defaultValue='${defaultValueArg?.getArgumentExpression()?.text}'")

		if (!isValid) {
			InspectionUtil.logIfEnabled(headerAnnotation.project, logger, "ERROR: Kotlin type-level header '$headerName' has missing, empty, or invalid defaultValue.")
...
// --- Kotlin Inspection ---
class WSHeadersOnTypeKotlinInspection : AbstractBaseUastLocalInspectionTool(), WSHeadersOnTypeInspectionLogic {
