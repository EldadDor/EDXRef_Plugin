...
interface WSHeaderOnMethodInspectionLogic {
  ...
  fun validateMethodHeader(project: Project, method: PsiMethod, holder: ProblemsHolder) {
    ...
    InspectionUtil.logIfEnabled(
      project,
      log,
      "Running WSHeaderOnMethod validation on ${containingClass.name}.${method.name}",
    )
    ...
    methodHeaders.forEach { (methodHeaderName, methodHeaderAnnotation) ->
      ...
      if (typeHeaders.containsKey(methodHeaderName)) {
        InspectionUtil.logIfEnabled(
          project,
          log,
          "WARN: Redundant header '$methodHeaderName' on method '${method.name}' also defined on type '${containingClass.name}'",
        )
        holder.registerProblem(
          method.nameIdentifier ?: method,
          MyBundle.message(
            "inspection.wsheaderonmethod.warn.redundant.method.header",
            methodHeaderName,
          ),
  ...
  fun validateKotlinMethodHeader(
    ...
    InspectionUtil.logIfEnabled(
      project,
      log,
      "Running WSHeaderOnMethod validation on Kotlin ${containingClass.name}.${function.name}",
    )
    ...
    functionHeaders.forEach { (methodHeaderName, methodHeaderAnnotation) ->
      // Rule 1: Validate defaultValue on setters
      if (isSetter) {
        validateKotlinSetterHeaderDefault(
          function,
          methodHeaderName,
      ...
      if (typeHeaders.containsKey(methodHeaderName)) {
        InspectionUtil.logIfEnabled(
          project,
          log,
          "WARN: Redundant header '$methodHeaderName' on Kotlin function '${function.name}' also defined on type '${containingClass.name}'",
        )
        holder.registerProblem(
  ...
  /**
   * Validates that if a defaultValue attribute exists on a @WSHeader on a setter method, it must be
   * non-empty.
   */
    ...
    if (defaultValueAttr is PsiLiteralExpression) {
      ...
      if (value != null && value.isEmpty()) {
        InspectionUtil.logIfEnabled(
          method.project,
          logger,
          "ERROR: Setter header '$headerName' on method '${method.name}' has an empty defaultValue.",
        )
        holder.registerProblem(
          method.nameIdentifier ?: method,
          MyBundle.message(
            "inspection.wsheaderonmethod.error.invalid.setter.defaultvalue",
            headerName,
            method.name,
          ),
    ...
    if (expr is KtStringTemplateExpression) {
      ...
      if (
        ...
        InspectionUtil.logIfEnabled(
          function.project,
          logger,
          "ERROR: Kotlin setter header '$headerName' on function '${function.name}' has an empty defaultValue.",
        )
        holder.registerProblem(
          function.nameIdentifier ?: function,
          MyBundle.message(
  ...
  private fun getMethodHeaders(method: PsiMethod, project: Project): Map<String, PsiAnnotation> {
    ...
    method.annotations
      .filter { it.hasQualifiedName(wsHeaderFqn) }
  ...
  private fun getKotlinFunctionHeaders(
    function: KtNamedFunction,
    project: Project,
  ): Map<String, KtAnnotationEntry> {
    ...
    function.annotationEntries
      .filter { it.shortName?.asString() == wsHeaderShortName }
      .forEach { headerAnnotation ->
...
// --- Java Inspection ---
class WSHeaderOnMethodJavaInspection :
  AbstractBaseJavaLocalInspectionTool(), WSHeaderOnMethodInspectionLogic {
  override val log = logger<WSHeaderOnMethodJavaInspection>()

  override fun getDisplayName(): String =
    MyBundle.message("inspection.wsheaderonmethod.displayname")
...
// --- Kotlin Inspection ---
class WSHeaderOnMethodKotlinInspection :
  AbstractBaseUastLocalInspectionTool(), WSHeaderOnMethodInspectionLogic {
