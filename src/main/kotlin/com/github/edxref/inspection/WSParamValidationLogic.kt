...
interface WSParamValidationLogic {
	...
	fun validateUrlParamsAgainstSetters(
		...
		methodParamMap.forEach { (effectiveParamName, mappedElements) ->
			mappedElements.forEach { element ->
				when (element) {
					is PsiMethod -> {
						validateMethodRules(element, effectiveParamName, methodsWithExplicitWsParamName, urlParams, wsParamFqn, holder, project, log)
					}

					is KtNamedFunction -> {
						validateKotlinFunctionRules(element, effectiveParamName, methodsWithExplicitWsParamName, urlParams, wsParamShortName, holder, project, log)
					}
				}
			}
		}
	...
	fun validateUrlParamsAgainstSetters(
		...
		validateUrlParamsAgainstSetters(
			project, urlParams, urlAttrValue, methods, emptyList(),
			wsConsumerAnnotation, holder, logger<WSParamValidationLogic>()
		)
	}
}
...
private fun validateMethodRules(
	...
	if (explicitWsParamName != null && explicitWsParamName !in urlParams) {
		...
		holder.registerProblem(
			...
			MyBundle.message("inspection.wsparam.error.wsparam.name.mismatch", explicitWsParamName, urlParams.joinToString()),
			ProblemHighlightType.ERROR
		)
	}
}
...
private fun validateKotlinFunctionRules(
	...
	if (explicitWsParamName != null && explicitWsParamName !in urlParams) {
		...
		val highlightElement = nameArg?.getArgumentExpression() ?: wsParamAnnotation ?: function.nameIdentifier ?: function
		holder.registerProblem(
			highlightElement,
			MyBundle.message("inspection.wsparam.error.wsparam.name.mismatch", explicitWsParamName, urlParams.joinToString()),
			ProblemHighlightType.ERROR
