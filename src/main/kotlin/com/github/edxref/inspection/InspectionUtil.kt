...
/**
 * Common utilities for inspections, especially for K2 Mode compatibility.
 * Centralizes settings access, annotation handling, and validation patterns.
 */
object InspectionUtil {
	...
	fun getWsConsumerAnnotationFqn(project: Project) =
		getSettings(project).wsConsumerAnnotationFqn.ifBlank { "com.github.edxref.annotations.WSConsumer" }

	fun getWebserviceConsumerFqn(project: Project) =
		getSettings(project).webserviceConsumerFqn.ifBlank { "com.github.edxref.annotations.WebserviceConsumer" }

	fun getPearlWebserviceConsumerFqn(project: Project) =
		getSettings(project).pearlWebserviceConsumerFqn.ifBlank { "com.github.edxref.annotations.PearlWebserviceConsumer" }
	...
	fun getWsHeadersFqn(project: Project) =
		getSettings(project).wsHeadersAnnotationFqn.ifBlank { "com.github.edxref.annotations.WSHeaders" }
	...
	fun getKotlinAnnotationStringAttribute(annotation: KtAnnotationEntry, attributeName: String): String? {
	...
	/**
	 * Checks if a Kotlin class/object has an annotation by short name.
	 * More reliable than light class conversion for K2 Mode.
	 */
	fun hasKotlinAnnotation(classOrObject: KtClassOrObject, annotationShortName: String): Boolean {
		return classOrObject.annotationEntries.any {
	 ...
	 */
	fun findKotlinAnnotation(classOrObject: KtClassOrObject, annotationShortName: String): KtAnnotationEntry? {
	 ...
	 */
	fun findKotlinFunctionAnnotation(function: KtNamedFunction, annotationShortName: String): KtAnnotationEntry? {
		return function.annotationEntries.find {
			it.shortName?.asString() == annotationShortName
		}
	}
	...
	/**
	 * Combined check for both WSConsumer annotation and webservice consumer inheritance.
	 * Falls back to light class only if needed for inheritance check.
	 */
