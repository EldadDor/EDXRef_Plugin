...
private fun getInternalApiAnnotationFqn(project: Project) = getSettings(project).internalApiAnnotationFqn.ifBlank { "com.github.edxref.annotations.InternalApi" }
...
interface InternalApiInspectionLogic {
    ...
    /**
     * Rule 1: Check if @InternalApi is incorrectly placed on an interface.
     * This is checked when visiting the annotation itself.
     */
    fun checkAnnotationPlacement(
        ...
        val annotationNameMatches = when (annotationElement) {
        ...
        val isInterface = when (annotatedElement) {
        ...
        if (isInterface) {
            logIfEnabled(project, log, "ERROR: @InternalApi found on interface '${(annotatedElement as? PsiNamedElement)?.name}'")
    ...
    /**
     * Rule 2: Check if a class implements an interface incorrectly marked with @InternalApi.
     * This is checked when visiting the class.
     */
    fun checkClassImplementation(
        ...
        if (foundBadInterface != null) {
            logIfEnabled(project, log, "ERROR: Class '${psiClass.name}' implements interface '${foundBadInterface!!.name}' which has @InternalApi")
            holder.registerProblem(
                elementToHighlight, // Highlight the class name
                MyBundle.message("inspection.internalapi.error.implementsannotatedinterface", foundBadInterface!!.name ?: "unknown interface"),
                ProblemHighlightType.ERROR
            )
        }
    }
...
// --- Java Inspection ---
class InternalApiJavaInspection : AbstractBaseJavaLocalInspectionTool(), InternalApiInspectionLogic {
    ...
    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean, session: LocalInspectionToolSession): PsiElementVisitor {
        return object : JavaElementVisitor() {
            ...
            override fun visitClass(psiClass: PsiClass) {
                ...
                checkClassImplementation(psiClass.project, psiClass, psiClass.nameIdentifier ?: psiClass, holder)
            }
        }
    }
}
...
class InternalApiKotlinInspection : AbstractKotlinInspection(), InternalApiInspectionLogic {
    ...
    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean): PsiElementVisitor {
        return object : KtVisitorVoid() {
            // Rule 1 Check: Visit the annotation entry
            override fun visitAnnotationEntry(annotationEntry: KtAnnotationEntry) {
                super.visitAnnotationEntry(annotationEntry)
                // Find the declaration (class/interface) the annotation is attached to
                val declaration = annotationEntry.getStrictParentOfType<KtDeclaration>()
                if (declaration is KtClassOrObject) { // Check if annotation is on a class/object/interface
                    checkAnnotationPlacement(annotationEntry.project, annotationEntry, declaration, holder)
                }
            }

            // Rule 2 Check: Visit the class/object definition
            override fun visitClassOrObject(classOrObject: KtClassOrObject) {
                super.visitClassOrObject(classOrObject)
                val psiClass = classOrObject.toLightClass() // Get Java representation for hierarchy check
                if (psiClass != null) {
