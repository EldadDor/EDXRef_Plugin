...
private fun getWsConsumerAnnotationFqn(project: Project) =
  getSettings(project).wsConsumerAnnotationFqn.ifBlank {
    "com.github.edxref.annotations.WSConsumer"
  }

private fun getWebserviceConsumerFqn(project: Project) =
  getSettings(project).webserviceConsumerFqn.ifBlank {
    "com.github.edxref.annotations.WebserviceConsumer"
  }

private fun getPropertyFqn(project: Project) =
  getSettings(project).propertyAnnotationFqn.ifBlank { "com.github.edxref.annotations.Property" }
...
interface WSInterfaceSpecificLogic {
    ...
    for (method in psiClass.methods) { // Only check methods directly in interface
      ...
      if ((isSetter || isGetter) && method.getAnnotation(propertyAnnotationFqn) == null) {
        logIfEnabled(
          project,
          log,
          "ERROR: Method '${method.name}' is missing the @$propertyAnnotationShortName annotation.",
        )
        ...
        holder.registerProblem(
          elementToHighlight,
          MyBundle.message(
            "inspection.wsinterfaceparam.error.missing.property.annotation",
            method.name,
            propertyAnnotationShortName,
          ),
  ...
  private fun validateHttpMethodAndGetters(
    ...
    val methodValue =
      if (methodAttrValue is PsiReferenceExpression) methodAttrValue.referenceName else null
  ...
  private fun validateGettersForBodyParam(
    ...
    val getters =
      psiClass.methods.filter { it.name.startsWith("get") && it.parameterList.parametersCount == 0 }

    if (getters.isNotEmpty()) {
      val bodyParamGetters =
        getters.filter { method ->
      ...
      val highlightElementForClassError =
        wsConsumerAnnotation.findAttributeValue("method") ?: psiClass.nameIdentifier ?: psiClass
      ...
      when (bodyParamGetters.size) {
        0 -> {
          logIfEnabled(
            project,
            log,
            "ERROR: Getters exist but none marked with isBodyParam=true in ${psiClass.name}",
          )
          holder.registerProblem(
        ...
        1 ->
          logIfEnabled(
            project,
            log,
            "OK: Found exactly one getter marked with isBodyParam=true in ${psiClass.name}",
          )
          ...
          logIfEnabled(
            project,
            log,
            "ERROR: Multiple getters marked with isBodyParam=true in ${psiClass.name}",
          )
          holder.registerProblem(
            highlightElementForClassError,
      ...
      logIfEnabled(
        project,
        log,
        "OK: No getters found in ${psiClass.name}, skipping isBodyParam check.",
      )
    }
  }
}
...
// --- Java Inspection ---
class WSInterfaceParamJavaInspection :
  AbstractBaseJavaLocalInspectionTool(), FullWSInterfaceValidationLogic {
  override val log = logger<WSInterfaceParamJavaInspection>() // Provide logger

  override fun getDisplayName(): String =
    MyBundle.message("inspection.wsinterfaceparam.displayname")
  ...
  override fun buildVisitor(
    ...
    return object : JavaElementVisitor() {
      override fun visitClass(psiClass: PsiClass) {
        ...
        val urlValue =
          if (urlAttrValue is PsiLiteralExpression && urlAttrValue.value is String)
...
// --- Kotlin Inspection ---
class WSInterfaceParamKotlinInspection :
  AbstractBaseUastLocalInspectionTool(), FullWSInterfaceValidationLogic {
      ...
      object : AbstractUastNonRecursiveVisitor() {
        override fun visitClass(node: UClass): Boolean {
          ...
          if (sourcePsi is KtClass && sourcePsi.isInterface()) {
            ...
            val wsConsumerAnnotation =
              psiClass.getAnnotation(getWsConsumerAnnotationFqn(project)) ?: return false

            if (!isImplementingInterface(psiClass, getWebserviceConsumerFqn(project))) return false
