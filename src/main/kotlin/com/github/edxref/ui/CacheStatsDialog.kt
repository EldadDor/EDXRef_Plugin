...
class CacheStatsDialog(private val project: Project, private val initialStats: Map<String, Any>) :
  ...
  override fun createCenterPanel(): JComponent {
    val mainPanel = JBPanel<JBPanel<*>>(BorderLayout()).apply { border = JBUI.Borders.empty(10) }
  ...
  private fun createHeaderPanel(): JComponent {
    ...
    val summaryLabel =
      JBLabel().apply {
        text =
          "<html><b>Cache Performance Summary:</b> ${hitRate}% hit rate from $totalRequests total requests</html>"
  ...
  private fun createTablePanel(): JComponent {
    ...
    val customTableModel =
      object : DefaultTableModel(arrayOf("Metric", "Value", "Description"), 0) {
    ...
    return JScrollPane(statsTable).apply { preferredSize = Dimension(550, 250) }
  }

  private fun createControlPanel(): JComponent {
    val panel = JBPanel<JBPanel<*>>(FlowLayout())

    val refreshButton = JButton("Refresh Now").apply { addActionListener { refreshStats() } }

    val autoRefreshCheckbox =
      JCheckBox("Auto Refresh (2s)", autoRefresh).apply {
        addActionListener {
    ...
    val clearCacheButton = JButton("Clear Cache").apply { addActionListener { clearCache() } }

    val exportButton = JButton("Export Stats").apply { addActionListener { exportStats() } }
  ...
  private fun updateTableData(stats: Map<String, Any>) {
    SwingUtilities.invokeLater {
      ...
      addStatRow(
        "Cache Misses",
        "${stats["cacheMisses"]}",
        "Number of cache misses requiring index lookup",
      )
      addStatRow(
        "XML Tag Cache",
        "${stats["xmlTagCacheSize"]} items",
        "Number of cached XML query tags",
      )
      addStatRow(
        "Interface Cache",
        "${stats["interfaceCacheSize"]} items",
        "Number of cached Java interfaces",
      )
  ...
  private fun exportStats() {
    ...
    val exportText = buildString {
      ...
      stats.forEach { (key, value) -> appendLine("$key: $value") }
