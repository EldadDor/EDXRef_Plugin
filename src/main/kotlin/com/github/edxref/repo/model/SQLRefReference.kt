...
class SQLRefReference(private var sqlRefId: String, private val project: Project) :
  Comparable<String> {

  private val xmlFiles: MutableMap<String, List<VirtualFile>> =
    MapMaker().initialCapacity(0).makeMap()
  private val classFiles: MutableMap<String, List<VirtualFile>> =
    MapMaker().initialCapacity(0).makeMap()
  ...
  private val xmlSmartPointersElements: MutableList<SmartPsiElementPointer<PsiElement>> =
    mutableListOf()
  private val classSmartPointersElements: MutableList<SmartPsiElementPointer<PsiElement>> =
    mutableListOf()
  private val utilClassSmartPointersElements:
    HashBasedTable<String, String, SmartPsiElementPointer<PsiElement>> =
    HashBasedTable.create()
  private val classPropertiedMethods: MutableMap<String, Map<String, PsiMethod>> =
    MapMaker().initialCapacity(0).makeMap()
  private val sqlSelectedColumns: MutableMap<String, String> =
    MapMaker().initialCapacity(0).makeMap()
  ...
  fun renameSQLRefId(sqlRefId: String) {
    ...
    classAnnoElements.forEach { classAnnoElement ->
      //				JamCommonUtil.setAnnotationAttributeValue(classAnnoElement as PsiAnnotation,
      // attributeId, sqlRefId)
    }
  ...
  fun addUtilClassCallInformation(
    refId: String,
    refToRef: String,
    classVF: VirtualFile,
  ...
  fun assignMethodPropertyInformationMap(
    methodPropertiesMap: Map<String, Map<String, PsiMethod>>
  ): SQLRefReference {
    classPropertiedMethods.putAll(methodPropertiesMap)
    return this
  }

  fun assignSqlSelectColumnsInformationMap(
    methodPropertiesMap: Map<String, String>
  ): SQLRefReference {
  ...
  private fun createAnnoRefSmartPointer(
    annoElement: PsiElement
